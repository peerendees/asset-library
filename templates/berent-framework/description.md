...
| `hooks/git-leitplanke.sh` | Haelt die Git-Regeln maschinell: verweigert Claude Commit und Push auf `main`, jeden Force-Push und den lokalen Merge nach `main` |
| `website-motor-leitplanken.md` | Wie BERENT-Websites gebaut werden: KI-nativ nach dem Website-Motor — sieben Leitplanken, BERENT-Festlegungen, Assistent auf der eigenen Website, Abnahmeliste (Referenz `peerendees/berentai-nativ`) |
...
| 1.8 | 2026-09-21 | Neue Schicht **`website-motor-leitplanken.md`** v1.0 (KI-native Websites: sieben Leitplanken, BERENT-Festlegungen, Abnahmeliste; Mechanismus aus der Bauanleitung „Der Website-Motor", Wortlaut eigen). `infrastructure-playbook.md` v1.5: §5a um **Node-Anwendungen mit Dockerfile** (Port-Label, Volume statt Host-Pfad, `/health` mit `gebaut`) und **„Woher weiß ich, wer ausliefert?"** ergaenzt — Beleg: www.berent.ai lief seit 07/2026 ueber Coolify, galt aber bis 21.09.2026 als Vercel-Deployment. `systems-register.md` v1.14: A11 + vier Labels fuer `berentai-nativ`. Nachtrag am selben Tag: Leitplanken v1.1 mit dem Abschnitt **Der Assistent auf der eigenen Website** (nur aus dem eigenen Bestand, keine Zahl ohne Beleg, Quellen unter jeder Antwort, kein Vektordienst solange Volltext reicht, Rechtliches aus dem Index, Deckel und ehrlicher Ausfall, Datenschutz vor dem Knopf, Fragen ohne Antwort als Themenplan). |
| 1.9 | 2026-09-22 | Neu: **`hooks/git-leitplanke.sh`** — ein PreToolUse-Hook, der Claude Commit und Push auf `main`, jeden Force-Push und den lokalen Merge nach `main` verweigert, mit Begruendung und dem richtigen Weg in der Fehlermeldung. `infrastructure-playbook.md` v1.6: Installation in §2.2, dazu zwei belegte Pull-Request-Fallen (gestapelter PR stirbt mit seinem Basiszweig; `gh pr merge` aus einem Arbeitsbaum meldet einen Scheinfehler). Anlass: ein Doku-Nachtrag landete trotz dreier schriftlicher Regeln direkt auf `main`. |
| 1.10 | 2026-09-22 | `hooks/git-leitplanke.sh` korrigiert: Das **Vorspulen von `main`** (`git pull --ff-only`, `git merge --ff-only origin/main`, `git merge origin/main`) ist erlaubt — es holt nach einem Merge auf GitHub nur den Stand nach und erzeugt keine Historie. Die erste Fassung blockierte es. `infrastructure-playbook.md` v1.7 haelt die Lehre fest: Ein Waechter, der die richtige Bewegung blockiert, wird umgangen. |
